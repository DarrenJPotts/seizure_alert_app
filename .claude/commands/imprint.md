...
At the start of any session that involves UI work, read `context/ui-registry.md` before writing any component. When building a new card, check how existing cards were built. When building a new button, check what button patterns already exist. When building a new status badge, match the exact values already in use.

The registry grows as the project grows. The more components are imprinted, the more consistent every new component becomes — because there is always a precise reference for what already exists.
...
**Border radius**
[List every border radius variant found and which components use it]
Recommendation: [which one to standardise on and why]

**Background colors**
[List every background value found — flag any hardcoded values]
Recommendation: [which token/constant should replace them]
...
Recommendation: [which Dimensions values to standardise on]

**Border colors**
[List every border color value found]
Recommendation: [which token/constant to standardise on]
...
[List every hardcoded value or non-token value found —
with the file and line where it appears]
These should be replaced with Dimensions constants or theme values.

### Recommended baseline
[The correct pattern for each property —
based on what the majority already uses correctly
and what CLAUDE.md defines]
```
