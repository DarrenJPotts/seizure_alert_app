...
- `context/build-plan.md` — the planned feature list
- `context/architecture.md` — architecture boundaries and patterns
- `context/code-standards.md` — coding conventions
- `CLAUDE.md` — design system rules
- `context/ui-registry.md` — existing UI patterns
...
- **Architecture boundaries** — is state management in GetxControllers? Are all Firestore calls going through FirestoreService? No direct `FirebaseFirestore.instance` calls in views or view models?
- **Design system** — are `Dimensions.*` constants used for all spacing? Are text styles from `Theme.of(context).textTheme`? No raw `fontSize` or hardcoded pixel values? No new colours introduced without justification?
- **Code standards** — naming conventions followed? DTOs have `fromMap`/`toMap`? All async calls wrapped in `try/catch`? Stream subscriptions cancelled in `onClose`?
- **Existing patterns** — does this feature introduce a new pattern when an existing one should have been used? Check `context/ui-registry.md` for UI components.

### Layer 3 — Is it production ready?

Check:

- Error handling — are Firestore and Firebase calls wrapped in `try/catch`? Does the UI surface errors via `GenericScreenStates.error`?
- Edge cases — empty states handled? Loading states shown? Null safety respected throughout?
- `flutter analyze` — run it. Are there any new warnings or errors introduced by this feature?
