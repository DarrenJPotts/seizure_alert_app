...
**What was built** — specific files created or modified, features completed, components added. Be precise. Not "built the alert flow" — "created lib/features/alert_history/alert_history_view.dart, lib/features/alert_history/view_models/alert_history_view_model.dart. Alert history screen with detail bottom sheet and map widget wired up end to end."

**Decisions made** — choices that would be hard to reverse or that future work depends on. Not implementation details — architectural choices. "Chose to use StatefulWidget for AlertResponsesWidget rather than a GetxController — it is scoped to one screen and its lifecycle matches the widget tree."
...
- `CLAUDE.md` — design system and project rules
- `context/project-overview.md` — what the app does
- `context/architecture.md` — stack, structure, patterns
